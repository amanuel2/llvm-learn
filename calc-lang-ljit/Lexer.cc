...
Token::TokenKind Lexer::peek () {
    Token Tok;
    const char* Tmp = BufferPtr;
    next (Tok);
    BufferPtr = Tmp;
    return Tok.getKind ();
}

Token::TokenKind toTokKind (llvm::StringRef Name) {
    if (Name == "with")
...
void Lexer::next (Token& Tok) {
    ...
    if (charinfo::isLetter (*BufferPtr)) {
        ...
        Token::TokenKind kind = toTokKind (Name);
        ...
        case '=': formToken (Tok, BufferPtr + 1, Token::Token::equals); break;
